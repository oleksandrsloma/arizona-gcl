...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedUser) {
        if (savedRole === 'leader') loginSuccess('Doni_Moore', {rank: 'Директор', level: 3}, true);
        else db.ref('users/' + savedUser).once('value').then(snap => { if(snap.exists()) loginSuccess(savedUser, snap.val(), false); });
    }
...
function performLoginCloud() {
    ...
    db.ref('users/' + nick).once('value').then(snap => {
        if (snap.val() && snap.val().pass === pass) {
            ...
            loginSuccess(nick, snap.val(), false);
...
function updateDashboardUI() {
    ...
    if(history.length === 0) list.innerHTML = "<tr><td colspan='3' style='text-align:center;color:#888'>Пусто</td></tr>";
...
function sendInternalReport(type) {
    ...
    // Пишем в Firebase, а бот оттуда прочитает и отправит Лидеру
...
function submitReportCloud() {
    ...
    db.ref('codes/' + id).once('value').then((snapshot) => {
        if (snapshot.val() === inputCode) {
            db.ref('codes/' + id).remove();
            
            const userRef = db.ref('users/' + currentUser);
            userRef.transaction((curr) => {
                if (curr) {
                    curr.balance = (curr.balance||0) + price;
                ...
                if(comm) {
                    db.ref(`users/${currentUser}/history`).push({op: `Заказ #${id}`, sum: `+${price}$`, date: new Date().toLocaleTimeString()});
...
function showToast(msg, type='info') {
    ...
    t.innerHTML = `<div class="toast-content"><p>${msg}</p></div>`;
    c.appendChild(t);
